 ...
 */
export const generateAtomicNameTradingPSBT = async (
	name,
	...
	if (!ownerOfName) {
		//don't add  change if we don't know the buyer address
		const transactionFee = getTransactionFee(fundingUtxoAddresses.length + 1);
		console.log('transactionFee', transactionFee);
		const changeAmount = Math.floor(
			totalInputAmount - storageFee - transactionFee - _transferPrice
		);
		if (changeAmount < 0) {
