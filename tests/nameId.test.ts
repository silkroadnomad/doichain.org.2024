...
test.describe('NameId Page', () => {
	test.beforeEach(async ({ page }) => {
		...
		await page.waitForFunction(
			() => {
				...
				return (
					stores &&
					stores.some((store: { ctx?: { helia?: any; libp2p?: any } }) => store && store.ctx && (store.ctx.helia || store.ctx.libp2p))
				);
			},
			{ timeout: 60000 }
		);
	...
	test('should display nameId details and meta tags', async ({ page }) => {
		...
		const title = await page.$eval('[data-testid="meta-title"]', (el) =>
			el.getAttribute('content')
		);
		const description = await page.$eval('[data-testid="meta-description"]', (el) =>
			el.getAttribute('content')
		);
		const image = await page.$eval('[data-testid="meta-image"]', (el) =>
			el.getAttribute('content')
		);
		const twitterCard = await page.$eval('[data-testid="meta-twitter-card"]', (el) =>
			el.getAttribute('content')
		);
		...
		expect(imageUrl).toMatch(
			/^(https:\/\/ipfs\.le-space\.de\/ipfs\/Qm|\/doichain_logo-min\.svg|https:\/\/doichain\.org\/favicon\.png)/
		);
	});
	...
	test('should handle invalid nameId gracefully', async ({ page }) => {
		...
		const errorText = await page.textContent('[data-testid="name-show-container"]', {
			timeout: 60000
		});
